...
import { knowledgeEntries, mockRetrievalResults, uploadedDocuments } from "@/mocks/knowledge-base";
import type {
  DocParseStatus,
  KnowledgeCategory,
  KnowledgeEntry,
  ...
  <PageContainer>
    <div class="kb-page">
      ...
      <div class="kb-mock-notice">
        ...
        <span
          >知识库 API 尚未在后端实现，当前展示为本地模拟数据。接入后端后将替换为真实检索结果。</span
        >
      </div>
              ...
              <span class="kb-tabs__count">
                {{
                  tab.key === "all"
                    ...
                    : knowledgeEntries.filter((e) => e.category === tab.key).length
                }}
              </span>
            </button>
          </nav>
              ...
              <tr v-for="entry in filteredEntries" :key="entry.id">
                ...
                <td>
                  <div class="kb-table__tags">
                    <span v-for="tag in entry.tags.slice(0, 3)" :key="tag" class="kb-tag">{{
                      tag
                    }}</span>
                    <span v-if="entry.tags.length > 3" class="kb-tag kb-tag--more"
              ...
              >
                ...
                <div class="kb-doc-item__status">
                  ...
                  <template v-else>
                    ...
                    <span
                      v-if="doc.parseStatus === 'indexed' && doc.chunks"
                      class="kb-doc-item__chunks"
                    >
                      {{ doc.chunks }} 个向量块
                    </span>
                  </template>
      ...
      <section class="kb-section">
        ...
        <div v-if="hasSearched" class="kb-retrieval-results">
          <p class="kb-retrieval-results__summary">
            ...
            <template v-else> 未找到相关片段，请尝试更换查询关键词 </template>
          </p>

          <div v-for="(result, idx) in retrievalResults" :key="result.id" class="kb-result-item">
            <div class="kb-result-item__head">
              <span class="kb-result-item__rank">#{{ idx + 1 }}</span>
              <span
                :class="[
                  'kb-result-item__score',
                  `kb-result-item__score--${scoreTone(result.score)}`,
                ]"
              >
            ...
            <blockquote class="kb-result-item__fragment">"{{ result.fragment }}"</blockquote>
