...
export function fetchProducts(params?: {
  ...
  return request<ProductResponse[]>(
    `/v1/commerce/products?${searchParams.toString()}`,
  )
}
...
export function fetchOrders(params?: {
  ...
  return request<OrderResponse[]>(
    `/v1/commerce/orders?${searchParams.toString()}`,
  )
}
...
export function fetchInventory(params?: {
  ...
  return request<InventoryItem[]>(
    `/v1/commerce/inventory?${searchParams.toString()}`,
  )
}
...
export async function fetchFunnelData(): Promise<FunnelData[]> {
  ...
  return [
    ...
    { label: "未激活", count: inactive, percentage: total ? Math.round((inactive / total) * 100) : 0 },
