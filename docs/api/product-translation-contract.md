...
商品管理页面将“选择目标语言”作为明确的翻译触发动作：所选语言尚无本地化内容且
Provider 已配置时，前端自动创建并确认现有翻译任务，不再要求用户额外点击翻译按钮。
后端仍完整创建 `AgentTask` 与 `ConfirmationTask`，不会绕过统一执行、Schema 校验、
失败状态和审计记录；相同浏览器会话中已经取得的译文直接复用。
