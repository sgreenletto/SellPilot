...
class KnowledgeIngestionService:
    ...
    async def import_package(self, data_dir: Path) -> dict:
        ...
        for m in msgs:
            ...
            (b_msgs if m.get("sender_type") == "buyer" else a_msgs).setdefault(sid, []).append(
                m.get("content", "")
            )
        ...
        for sid in sorted(set(b_msgs) | set(a_msgs)):
            ...
            await self._make(
                f"FAQ #{sid}",
                self._faq(
                    q, a, s.get("language", ""), s.get("intent", ""), s.get("risk_level", "")
                ),
                "faq",
        ...
        while True:
            ...
            for chunk, emb in zip(batch, embs, strict=True):
