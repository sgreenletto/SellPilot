...
class KnowledgeIngestionService:
    ...
    def _prod(row: dict) -> str:
        ...
        return (
            ...
            f"Site: {site} | Price: {price} {currency} | Sales: {sales}"
        )
    ...
    def _rev(row: dict) -> str:
        ...
        sentiment = row.get("sentiment_hint", "")
        rating = row.get("rating", "")
        return (
            f"[User Review - {row.get('language', '')}]\n"
            f"Content: {row.get('content', '')}\n"
            f"{zh_line}"
            f"Issue: {issue} | Sentiment: {sentiment} | Rating: {rating}/5"
        )
    ...
    async def _make(
        self, title: str, content: str, cat: str, src: str,
    ) -> KnowledgeDocument:
        doc = KnowledgeDocument(
            title=title, file_type="csv",
            file_size_bytes=len(content.encode()),
            category=cat, status="indexed", source=src,
            chunk_count=1, created_by=_ADMIN,
        )
        self.session.add(doc)
        await self.session.flush()
        self.session.add(KnowledgeChunk(
            document_id=doc.id, chunk_index=0,
            content=content, chunk_size=len(content),
        ))
        return doc
    ...
    async def import_package(self, data_dir: Path) -> dict:
        ...
        for row in prods:
            await self._make(
                row.get("title", ""), self._prod(row), "product",
        ...
        while True:
            ...
            for chunk, emb in zip(batch, embs, strict=False):
                doc = await self.session.get(
                    KnowledgeDocument, chunk.document_id,
                )
